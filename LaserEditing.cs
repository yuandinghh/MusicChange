...
namespace MusicChange
{
	...
	{       // Windows API 函数
		...
		private void LaserEditing_Load(object sender, EventArgs e)
		{
			...
			Core.Initialize(); // 初始化 LibVLCSharp
			try
			{
				// 初始化 LibVLC
				//Core.Initialize();
			...
			{
				MessageBox.Show($"初始化 LibVLC 失败: {ex.Message}", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
			}
...
MessageBox.Show("删除子目录失败: " + ex.Message);
	...
	private void buttonx11_Click(object sender, EventArgs e)
	{
		...
		//try
		//{
		//	//if (videoView1 != null && _libVlc != null) {
		//	// 创建新的 MediaPlayer
		...
		//	// 创建并播放媒体
		//	Media media1 = new Media( _libVLC, filePath, FromType.FromPath);
